...
const css = `
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .form-title { font-family:'Playfair Display',Georgia,serif; font-size:22px; color:#E8EDF5; margin-bottom:6px; }
  .form-sub { font-size:13px; color:#8A95AB; margin-bottom:26px; line-height:1.6; }
  ...
  .btn { width:100%; background:#3B82F6; color:#fff; font-size:15px; font-weight:600; padding:12px; border-radius:9px; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:10px; transition:opacity 0.15s; margin-top:16px; }
  .btn:hover:not(:disabled) { opacity:0.9; }
  .btn:disabled { opacity:0.5; cursor:not-allowed; }
  .status { font-size:13px; line-height:1.6; margin-top:14px; text-align:center; border-radius:8px; padding:12px 16px; }
  ...
  .note { font-size:12px; color:#4A546A; margin-top:8px; }
...
const ERR = {
  ...
  expired_token: "That link expired (15-minute window) — request a new one below.",
  account_canceled: "Your account has been canceled. Email alerts@landusealert.com to resubscribe.",
...
export default function Login() {
  ...
  const [status, setStatus] = useState(urlError ? { type: "err", msg: ERR[urlError] || "Something went wrong." } : null);
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!r.ok) setStatus({ type: "err", msg: data.error || "Something went wrong." });
      else { setSent(true); setStatus({ type: "ok", msg: `Check ${email} — your sign-in link expires in 15 minutes.` }); }
    } catch {
        ...
        <div className="form-card">
          ...
          {!sent && (
            <form onSubmit={submit}>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane@smithdev.com" required autoFocus />
              <p className="note">Link expires in 15 minutes.</p>
          ...
          <p className="signup-link">No account? <Link href="/signup">Get started</Link></p>
        </div>
      </div>
    </>
  );
